...
class RecruitmentView(discord.ui.View):
    ...
    @discord.ui.button(label="⭐ Postuler maintenant ⭐", style=discord.ButtonStyle.success, custom_id="kawail_apply_final_v5")
...
class AdminView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try: await user.send("✅ Tu as été **accepté** sur le serveur **kawail_fps** ! Félicitations !")
        except: pass
    ...
    async def refuse(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        user = await self.bot.fetch_user(self.user_id)
        try: await user.send("❌ Oh non, ta candidature pour **kawail_fps** n'a pas été retenue.")
        except: pass
...
class CandidatureModal(discord.ui.Modal, title="Dossier Staff Kawail_FPS"):
    pseudo = discord.ui.TextInput(label="Pseudo & Âge", placeholder="Kawail_FPS, 17 ans", required=True)
    dispo = discord.ui.TextInput(label="Tes disponibilités", placeholder="Soirs et week-end", required=True)
...
class MyBot(discord.Client):
    def __init__(self):
        intents = discord.Intents.all() # Nécessite d'activer les 3 interrupteurs sur le portail Discord
    ...
    async def check_recruitment_post(self):
        ...
        # Vérification si l'embed existe déjà
        async for m in channel.history(limit=5):
            if m.author.id == self.user.id and m.embeds: return 
...
@bot.tree.command(name="annuler", description="Annule le dernier choix pour un membre")
async def annuler(interaction: discord.Interaction, membre: discord.Member):
    ...
    await interaction.response.send_message(f"🔄 Action annulée pour {membre.name}. Son dossier est repassé en attente.")
