...
def github_webhook():
    if not bot.is_ready():
        return "Waiting for bot login...", 503
...
class RecruitmentView(discord.ui.View):
    ...
    @discord.ui.button(label="⭐ Postuler maintenant ⭐", style=discord.ButtonStyle.success, custom_id="kawail_v27")
...
class CandidatureModal(discord.ui.Modal, title="Dossier Staff Kawail_FPS"):
    ...
    dispo = discord.ui.TextInput(label="Disponibilités", placeholder="Tes horaires")
    exp = discord.ui.TextInput(label="Expériences", style=discord.TextStyle.paragraph)
    apport = discord.ui.TextInput(label="Ton apport", style=discord.TextStyle.paragraph)
...
async def send_github_update(author, message, url, repo, branch):
    ...
    if not channel: 
        channel = await guild.create_text_channel(GITHUB_CHAN_NAME, category=guild.get_channel(CAT_INFO_ID))
    ...
    async def on_message(self, message):
        ...
        if not is_staff:
            if "http" in message.content or "discord.gg" in message.content:
                ...
                return await message.channel.send(f"⚠️ {message.author.mention}, liens interdits !", delete_after=3)
            
            if any(word in message.content.lower() for word in BANNED_WORDS):
                await message.delete()
                return await message.channel.send(f"⚠️ {message.author.mention}, langage inapproprié !", delete_after=3)
